...
(defstruct project
  ...
  (test nil :type (or null list))
  (package nil :type (or null list))
  ...
  ;; Plist controlling the persistent Lisp daemon (`clpm repl'):
...
(defstruct locked-registry
  ...
  ;; Quicklisp-only snapshot pins (SHA-256 hex digests of systems.txt and releases.txt).
  (quicklisp-systems-sha256 nil :type (or null string))
  (quicklisp-releases-sha256 nil :type (or null string)))
      ...
      ((and (consp form) (eq (car form) :quicklisp))
       ...
       (loop for (key val) on (cdr form) by #'cddr do
         (case key
           ...
           (:name (setf (registry-ref-name ref) val))
           (:trust (setf (registry-ref-trust ref) val))
           (:systems-sha256 (setf (registry-ref-quicklisp-systems-sha256 ref) val))
           (:releases-sha256 (setf (registry-ref-quicklisp-releases-sha256 ref) val)))))
      (t
      ...
      (case key
        ...
        (:license
         (when (and val (not (stringp val)))
           (error "Invalid :license value: expected a string or nil, got ~S" val))
         (setf (project-license project) val))
        (:homepage
         (when (and val (not (stringp val)))
           (error "Invalid :homepage value: expected a string or nil, got ~S" val))
         (setf (project-homepage project) val))
        (:description
         (when (and val (not (stringp val)))
           (error "Invalid :description value: expected a string or nil, got ~S" val))
         (setf (project-description project) val))
        (:systems (setf (project-systems project) val))
        (:run (setf (project-run project) val))
        (:test (setf (project-test project) val))
        (:package (setf (project-package project) val))
        (:depends
        ...
        (:repl
         (unless (listp val)
           (error "Invalid :repl value: expected a plist, got ~S" val))
         (setf (project-repl project) val))))
    project))
  ...
  (let ((reg (make-locked-registry)))
    (loop for (key val) on (cdr form) by #'cddr do
      (case key
        ...
        (:systems-sha256 (setf (locked-registry-quicklisp-systems-sha256 reg) val))
        (:releases-sha256 (setf (locked-registry-quicklisp-releases-sha256 reg) val))))
    reg))
...
(defun serialize-locked-registry (reg)
  ...
  `(:registry
    ...
    ,@(when (and (eq (locked-registry-kind reg) :quicklisp)
                 (locked-registry-quicklisp-systems-sha256 reg))
        (list :systems-sha256 (locked-registry-quicklisp-systems-sha256 reg)))
    ,@(when (and (eq (locked-registry-kind reg) :quicklisp)
                 (locked-registry-quicklisp-releases-sha256 reg))
        (list :releases-sha256 (locked-registry-quicklisp-releases-sha256 reg)))))
...
(defun %serialize-registry-ref (r)
  (let ((form `(,(registry-ref-kind r)
                :url ,(registry-ref-url r)
                :name ,(registry-ref-name r)
                :trust ,(registry-ref-trust r))))
    (when (and (eq (registry-ref-kind r) :quicklisp)
               (registry-ref-quicklisp-systems-sha256 r))
      (setf form (append form (list :systems-sha256
                                    (registry-ref-quicklisp-systems-sha256 r)))))
    (when (and (eq (registry-ref-kind r) :quicklisp)
               (registry-ref-quicklisp-releases-sha256 r))
      (setf form (append form (list :releases-sha256
                                    (registry-ref-quicklisp-releases-sha256 r)))))
    form))
...
(defun serialize-project (project)
  ...
  `(:project
    ...
    :depends ,(mapcar #'%serialize-dependency (project-depends project))
    :dev-depends ,(mapcar #'%serialize-dependency (project-dev-depends project))
    :test-depends ,(mapcar #'%serialize-dependency (project-test-depends project))
    :registries ,(mapcar #'%serialize-registry-ref (project-registries project))
    ...
    ,@(when (project-repl project)
        (list :repl (project-repl project)))))
