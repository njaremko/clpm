...
(defun %sexpr-replace-source-range (text start end replacement)
  (concatenate 'string
               (subseq text 0 start)
               replacement
               (subseq text end)))

(defun %sexpr-write-source-text (pathname text)
...
(defun %sexpr-symbol-token (symbol package)
  (let ((*package* package)
        (*print-case* :downcase)
        (*print-pretty* nil)
        (*print-circle* t))
...
(defun %sexpr-read-binding-symbol (name package)
  (let ((*package* package)
        (*read-eval* nil))
    (let ((symbol (%read-form name)))
      (unless (and (symbolp symbol)
                   (not (%sexpr-effect-constant-symbol-p symbol)))
        (error "binding name must read as a non-constant symbol"))
      symbol)))

(defun %sexpr-introduce-let-replacement (name-symbol selected-text package)
  (let ((name-token (%sexpr-symbol-token name-symbol package)))
...
(defun %sexpr-edited-source-document (pathname text server)
  (let ((tmp-path (make-pathname
                   :name (format nil ".sexpr-edit-~A"
                                 (random (expt 2 32)))
                   :type "lisp"
                   :defaults pathname)))
    (unwind-protect
         (progn
           (%sexpr-write-source-text tmp-path text)
           (clpm.sexpr-edit:read-source-document
            (namestring tmp-path)
            :initial-package-name (%sexpr-current-package-name server)))
      (ignore-errors (delete-file tmp-path)))))

(defun %sexpr-unique-symbols-by-name (symbols)
  (let ((seen '())
        (result '()))
...
(defun %sexpr-reference-symbol-p (symbol)
  (and (symbolp symbol)
       (not (%sexpr-effect-constant-symbol-p symbol))))

(defun %sexpr-form-symbol-references (form)
  (let ((references '()))
    (labels ((binding-name (binding)
               (cond
                 ((symbolp binding) binding)
             ...
             (let-binding-value (binding)
               (and (consp binding) (second binding)))
             (walk-body (forms bound)
               (dolist (body-form forms)
                 (walk body-form bound)))
             (walk-let (elements bound sequential-p)
               (let ((body-bound bound))
                 (dolist (binding (second elements))
                   (let ((value (let-binding-value binding))
                         (name (binding-name binding)))
                     (when value
                       (walk value (if sequential-p body-bound bound)))
                     (when name
                       (push name body-bound))))
                 (walk-body (cddr elements) body-bound)))
             (walk (current bound)
               (cond
                 ((%sexpr-reference-symbol-p current)
                  (unless (member current bound)
                    (push current references)))
                 ((atom current) nil)
                 ((%sexpr-quoted-form-p current) nil)
                 (t
                  (let ((elements (%sexpr-proper-list-elements current)))
                    (when elements
                      (let ((operator (%sexpr-form-operator-name current)))
                        (cond
                          ((member operator '("FUNCTION" "DECLARE")
                           ...
                           nil)
                          ((string= operator "LAMBDA")
                           (walk-body (cddr elements)
                                      (append (%lambda-list-bindings
                                               (second elements))
                                              bound)))
                          ((string= operator "LET")
                           (walk-let elements bound nil))
                          ((string= operator "LET*")
                           (walk-let elements bound t))
                          (t
                           (loop for child in (rest elements)
                                 do (walk child bound)))))))))))
      (walk form nil))
    (%sexpr-unique-symbols-by-name (nreverse references))))

(defun %sexpr-visible-lexical-names (top-form child-path)
  (let* ((scope (%sexpr-scope-at-child-path top-form child-path))
         (entries (getf scope :lexical)))
    (mapcar (lambda (entry)
              (%json-getf entry "name"))
            entries)))

(defun %sexpr-free-lexical-symbols (top-form selected child-path)
  (let ((visible-names (%sexpr-visible-lexical-names top-form child-path)))
    (remove-if-not
     (lambda (symbol)
       (member (symbol-name symbol) visible-names :test #'string=))
     (%sexpr-form-symbol-references selected))))

(defun %sexpr-extracted-defun-text (name-symbol parameters body-text package)
  (let ((name-token (%sexpr-symbol-token name-symbol package))
        (parameter-tokens
          (mapcar (lambda (symbol)
                    (%sexpr-symbol-token symbol package))
                  parameters)))
    (format nil "(defun ~A (~{~A~^ ~})~%  ~A)"
            name-token parameter-tokens body-text)))

(defun %sexpr-call-text (name-symbol arguments package)
  (let ((name-token (%sexpr-symbol-token name-symbol package))
        (argument-tokens
          (mapcar (lambda (symbol)
                    (%sexpr-symbol-token symbol package))
...
(defun %sexpr-child-edit-provenance-json (operation dry-run path)
  (%json-object
   ...
   "timestamp_universal" (get-universal-time)
   "reason" operation
   "operations" (%json-array (list operation))
   "committed" (%sexpr-boolean-json (not dry-run))
   "source_comments_inserted" :false
   "changed_forms" (%json-array (list path))))

(defun %dispatch-sexpr-introduce-let (server params id)
  (let ((path (%json-getf params "path"))
        (name (%json-getf params "name"))
        (dry-run (%json-true-p (%json-getf params "dry_run"))))
    (cond
                                ...
                                (let* ((child-path (%sexpr-child-path path))
                                       ...
                                       (end
                                         (clpm.sexpr-edit:source-child-span-end
                                          span))
                                       (selected-text
                                         (subseq before start end))
                                       (name-symbol
                                         (%sexpr-read-binding-symbol name pkg))
                                       (replacement
                                         (%sexpr-introduce-let-replacement
                                          name-symbol selected-text pkg))
                                       (after
                                         (%sexpr-replace-source-range
                                          before start end replacement)))
...
(defun %dispatch-sexpr-extract-function (server params id)
  (let ((path (%json-getf params "path"))
        (name (%json-getf params "name"))
        (dry-run (%json-true-p (%json-getf params "dry_run"))))
    (cond
                                ...
                                (let* ((child-path (%sexpr-child-path path))
                                  ...
                                  (multiple-value-bind (selected present-p)
                                      (%sexpr-form-at-child-path
                                       top-form child-path)
                                    (unless present-p
                                      (error "child path does not resolve to a source form"))
                                    (let* ((span
                                             (clpm.sexpr-edit:source-form-child-span
                                              source-form child-path))
                                           (before
                                           ...
                                           (selected-text
                                             (subseq before start end))
                                           (name-symbol
                                             (%sexpr-read-binding-symbol
                                              name pkg))
                                           (free-symbols
                                             (%sexpr-free-lexical-symbols
                                              top-form selected child-path))
                                           (call-text
                                             (%sexpr-call-text
                                              name-symbol free-symbols pkg))
                                           (defun-text
                                             (%sexpr-extracted-defun-text
                                              name-symbol free-symbols
                                              selected-text pkg))
                                           (replaced
                                             (%sexpr-replace-source-range
                                              before start end call-text))
                                           (after
                                             (%sexpr-replace-source-range
                                              replaced
                                              (clpm.sexpr-edit:source-form-start
                                               source-form)
                                              (clpm.sexpr-edit:source-form-start
                                               source-form)
                                              (concatenate 'string
                                                           ...
                                                           (string #\Newline))))
                                           (checked
                                             (%sexpr-edited-source-document
                                              pathname after server)))
                                      (if (clpm.sexpr-edit:source-document-diagnostics
                                           checked)
                                          (%success-response
                                           id
...
(%register-method
 (make-method-spec
  :name "sexpr-introduce-let"
  :summary "Wrap a selected source subform in a LET binding."
  :doc "Required: `path' and `name'. The path must include `child_path' so
the operation replaces exactly one nested source form with `(let ((name old))
name)'. Optional `dry_run' validates and reports the replacement without
writing. The file is only written after the edited source reads cleanly."
  :params (list (list :name "path" :type :object :required t
                      :description "Source path object with file, selector, and child_path.")
                (list :name "name" :type :string :required t
                      :description "Binding name to introduce, read in the source package.")
                (list :name "dry_run" :type :boolean :required nil
                      :description "Validate without writing the file."))
  :handler
  (lambda (server params id ctx)
...
(%register-method
 (make-method-spec
  :name "sexpr-extract-function"
  :summary "Extract a selected source subform into a new DEFUN."
  :doc "Required: `path' and `name'. The path must include `child_path'.
The first implementation computes visible lexical variables syntactically,
inserts a new DEFUN before the enclosing top-level form, replaces the
selected subform with one call, and validates the edited source before
writing. Optional `dry_run' reports the edit without writing."
  :params (list (list :name "path" :type :object :required t
                      :description "Source path object with file, selector, and child_path.")
                (list :name "name" :type :string :required t
                      :description "Function name to define, read in the source package.")
                (list :name "dry_run" :type :boolean :required nil
                      :description "Validate without writing the file."))
  :handler
  (lambda (server params id ctx)
...
(defun %sexpr-json-string-array (value)
  (when (%json-array-p value)
    (let ((items (%json-array-items value)))
      (when (every #'stringp items)
        items))))

(defun %sexpr-generated-test-name (raw fallback)
  (let ((name (or raw fallback)))
    (if (and (stringp name) (plusp (length name)))
        name
        fallback)))

(defun %sexpr-readable-form-string (value package)
...
(defun %sexpr-test-style (server)
  (let* ((root (and server (server-project-root server)))
         (test-dir (and root (merge-pathnames "test/" root))))
    (if (and test-dir
             (probe-file test-dir)
             (directory (merge-pathnames "*.lisp" test-dir)))
        "lightweight_sbcl_script"
        "lightweight_sbcl_script")))

(defun %sexpr-output-pathname (file server)
  (let* ((defaults (or (and server (server-project-root server))
                       (uiop:getcwd)))
         (pathname (merge-pathnames file defaults)))
    (ensure-directories-exist pathname)
    pathname))

(defun %sexpr-load-file-forms-text (files)
  (with-output-to-string (s)
    (dolist (file files)
      (format s "(load #P~S :verbose nil :print nil)~%"
              (namestring (truename file))))))

(defun %sexpr-generated-test-prologue (test-name package-name load-files)
  (format nil ";;;; Generated by SexprEdit for ~A.

(in-package ~S)

(defun sexpr-generated-fail (format-control &rest args)
...
"
          test-name
          package-name
          (%sexpr-load-file-forms-text load-files)))

(defun %sexpr-repl-example-test-source (test-name package form-text values
                                        load-files)
  (let* ((package-name (package-name package))
         (expected (%sexpr-readable-form-string values package)))
    (concatenate
     'string
     (%sexpr-generated-test-prologue test-name package-name load-files)
     (format nil ";;; Action
(let ((actual (multiple-value-list ~A))
      (expected '~A))
  ;;; Assertion
  (unless (equal actual expected)
    (sexpr-generated-fail \"~A expected ~~S, got ~~S\" expected actual)))

(format t \"~A passed~%\")
"
             form-text expected test-name test-name))))

(defun %sexpr-definition-assertion-text (kind name package-name test-name)
  (cond
    ((string= kind "defpackage")
     (format nil "(unless (find-package ~S)
  (sexpr-generated-fail \"~A expected package ~A to exist\"))"
             name test-name name))
    ((member kind '("defclass" "define-condition") :test #'string=)
     (format nil "(let ((symbol (find-symbol ~S ~S)))
  (unless (and symbol (find-class symbol nil))
    (sexpr-generated-fail \"~A expected class ~A to be defined\")))"
             name package-name test-name name))
    ((member kind '("defvar" "defparameter" "defconstant") :test #'string=)
     (format nil "(let ((symbol (find-symbol ~S ~S)))
  (unless (and symbol (boundp symbol))
    (sexpr-generated-fail \"~A expected variable ~A to be bound\")))"
             name package-name test-name name))
    (t
     (format nil "(let ((symbol (find-symbol ~S ~S)))
  (unless (and symbol (fboundp symbol))
    (sexpr-generated-fail \"~A expected function ~A to be fbound\")))"
             name package-name test-name name))))

(defun %sexpr-definition-test-source (test-name package source-file source-form)
  (let* ((package-name (or (clpm.sexpr-edit:source-form-package source-form)
                           (package-name package)))
         (name (clpm.sexpr-edit:source-form-name source-form))
         (kind (clpm.sexpr-edit:source-form-kind source-form)))
    (concatenate
     'string
     (%sexpr-generated-test-prologue test-name package-name (list source-file))
     (format nil ";;; Action
~A
...
(format t \"~A passed~%\")
"
             (%sexpr-definition-assertion-text kind name package-name
                                               test-name)
             test-name))))

(defun %sexpr-sbcl-program ()
  #+sbcl
  (let ((runtime sb-ext:*runtime-pathname*))
    (if runtime (namestring runtime) "sbcl"))
  #-sbcl
  "sbcl")

(defun %sexpr-run-generated-test (output-file)
  (let ((program (%sexpr-sbcl-program))
        (file (namestring (truename output-file))))
    (multiple-value-bind (stdout stderr exit-code)
        (clpm.platform:run-program (list program "--script" file)
                                   :output :string
                                   :error-output :string)
      (%json-object
       "success" (%sexpr-validation-success-p (zerop exit-code))
       "command" (%json-array (list program "--script" file))
       "exit_code" exit-code
       "stdout" stdout
       "stderr" stderr))))

(defun %dispatch-sexpr-generate-test (server params id)
  (let* ((form-text (%json-getf params "form"))
         (path (%json-getf params "path"))
         (output-file (%json-getf params "output_file"))
         (raw-load-files (%json-getf params "load_files"))
         (load-files (%sexpr-json-string-array raw-load-files))
         (package-name (or (%json-getf params "package")
                           (%sexpr-current-package-name server)))
         (test-name
           (%sexpr-generated-test-name (%json-getf params "test_name")
                                       "sexpr-generated-test"))
         (dry-run (%json-true-p (%json-getf params "dry_run")))
         (validate (not (eq (%json-getf params "validate") :false))))
    (cond
      ((not (stringp output-file))
       (%error-response id "protocol-error" "missing `output_file' param"))
      ((and raw-load-files (null load-files))
       (%error-response id "protocol-error"
                        "`load_files' must be an array of strings"))
      ((and (not (stringp form-text))
            (not (%json-object-p path)))
       (%error-response id "protocol-error"
                        "provide either `form' or `path'"))
      ((and (stringp form-text) (%json-object-p path))
       (%error-response id "protocol-error"
                        "`form' and `path' are mutually exclusive"))
      (t
       (handler-case
           (let* ((package (%reader-package-for-server server package-name))
                  (style (%sexpr-test-style server))
                  (pathname (%sexpr-output-pathname output-file server))
                  (source
                    (if form-text
                        (progn
                          ...
                          (let* ((*package* package)
                                 (*read-eval* nil)
                                 (form (%read-form form-text))
                                 (values (multiple-value-list (eval form))))
                            (%sexpr-repl-example-test-source
                             test-name package (%sexpr-trim-source form-text)
                             values load-files)))
                        (let ((file (%sexpr-path-field path "file")))
                          (unless (stringp file)
                            (error "`path.file' must be a string"))
                          (multiple-value-bind (document error-response)
                              (%sexpr-read-document server file id)
                            (when error-response
                              (return-from %dispatch-sexpr-generate-test
                                error-response))
                            (let* ((actual-file
                                     (namestring
                                      (clpm.sexpr-edit:source-document-pathname
                                       document)))
                                   (forms (%sexpr-selected-forms document
                                                                 path)))
                              (cond
                                ((null forms)
                                 (return-from
                                  %dispatch-sexpr-generate-test
                                  (%error-response id "eval-error"
                                                   "no form matched source path")))
                                ((rest forms)
                                 (return-from
                                   ...
                                   (%json-object
                                    ...
                                    (%json-array
                                     (mapcar
                                      ...
                                      forms))))))
                                (t
                                 (%sexpr-definition-test-source
                                  test-name package actual-file
                                  (first forms))))))))))
             (unless dry-run
               (%sexpr-write-source-text pathname source))
             (%success-response
              id
              (%json-object
               "status" "ok"
               "kind" (if form-text "repl_example" "definition")
               "style" style
               "test_name" test-name
               "output_file" (namestring pathname)
               "dry_run" (%sexpr-boolean-json dry-run)
               "source" source
               "validation"
               (if (and validate (not dry-run))
                   (%sexpr-run-generated-test pathname)
                   (%json-object
                    "success" (%sexpr-validation-success-p (not validate))
                    "skipped" (%sexpr-boolean-json t)
                    "reason" (if dry-run "dry-run" "validate=false")))
               "provenance"
               (%json-object
                "created_by" "sexpr-edit"
                "timestamp_universal" (get-universal-time)
                "reason" "test generation"
                "operations" (%json-array (list "generate-test"))
                "committed" (%sexpr-boolean-json (not dry-run))
                "source_comments_inserted" :false))))
         (error (c)
           (%error-response id "eval-error" (princ-to-string c))))))))
...
(%register-method
 (make-method-spec
  :name "sexpr-generate-test"
  :summary "Generate a lightweight SBCL script test from a form or definition."
  :doc "Required: `output_file' and either `form' or `path'. With `form',
the daemon evaluates the form once in the requested package, captures the
multiple-value result, writes a deterministic smoke test, and runs it with
SBCL by default. With `path', the daemon selects one source definition,
generates a load-and-definition-present smoke test, and validates it. Optional
`load_files' are loaded before captured examples and in the generated script.
The method returns the generated source, style, provenance, and validation
transcript; ambiguous paths return candidates and leave files unchanged."
  :params (list (list :name "output_file" :type :string :required t
                      :description "Test script path to write.")
                (list :name "form" :type :string :required nil
                      :description "REPL form to capture as a deterministic example.")
                (list :name "path" :type :object :required nil
                      :description "Source definition path for a definition smoke test.")
                (list :name "test_name" :type :string :required nil
                      :description "Human-readable generated test name.")
                (list :name "package" :type :string :required nil
                      :description "Reader/evaluation package for `form'.")
                (list :name "load_files" :type :array :required nil
                      :description "Files to load before evaluating and in the script.")
                (list :name "validate" :type :boolean :required nil
                      :description "Run the generated script unless false.")
                (list :name "dry_run" :type :boolean :required nil
                      :description "Return source without writing or validating."))
  :handler
  (lambda (server params id ctx)
    (declare (ignore ctx))
    (%dispatch-sexpr-generate-test server params id))))
