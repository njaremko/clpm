;;;; test/repl-compat-test.lisp - cross-impl primitives
;;;;
;;;; The bridge is portable across SBCL/CCL/ECL via clpm.repl.compat.
...
(let* ((flag nil)
       (th (clpm.repl.compat:make-thread
            (lambda () (setf flag :ran))
            :name "compat-test-1")))
  (clpm.repl.compat:join-thread th)
  (assert-eql :ran flag)
  (assert-true (not (clpm.repl.compat:thread-alive-p th))
...
(let* ((latch (make-array 1 :initial-element nil))
  ...
  (loop for i from 0 below 50
        while (clpm.repl.compat:thread-alive-p th)
        do (sleep 0.05))
  (assert-true (not (clpm.repl.compat:thread-alive-p th))
...
(let ((th (clpm.repl.compat:make-thread
           (lambda () (loop (sleep 0.05)))
           :name "compat-test-term")))
  (sleep 0.05)
  (assert-true (clpm.repl.compat:thread-alive-p th)
               "thread died unexpectedly before termination")
  (clpm.repl.compat:terminate-thread th)
  (loop for i from 0 below 50
        while (clpm.repl.compat:thread-alive-p th)
        do (sleep 0.05))
  (assert-true (not (clpm.repl.compat:thread-alive-p th))
...
(let* ((mut (clpm.repl.compat:make-mutex :name "compat-test"))
       (counter 0)
       (n 8)
       (per-thread 1000)
       (threads
         (loop repeat n
               collect (clpm.repl.compat:make-thread
                        (lambda ()
                          (loop repeat per-thread do
                            (clpm.repl.compat:with-mutex (mut)
                              (incf counter))))
...
(let* ((box (clpm.repl.compat:make-mailbox))
       (producers 4)
       (per-producer 25)
  ...
  (dolist (th producer-threads)
    (clpm.repl.compat:join-thread th))
  (dotimes (i total)
    (push (clpm.repl.compat:receive-message box) received))
...
(let ((frames (clpm.repl.compat:list-backtrace)))
  (assert-true (listp frames) "list-backtrace did not return a list")
  (dolist (f frames)
    (princ-to-string f)))
(format t "  list-backtrace OK (~D frames)~%"
        (length (clpm.repl.compat:list-backtrace)))

(format t "~%REPL compat layer tests PASSED on ~A!~%"
        (clpm.repl.compat:host-impl))
(sb-ext:exit :code 0)
