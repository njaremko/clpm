;;;; test/registry-trust-cmd-test.lisp - Registry trust subcommand tests
...
(format t "Testing `clpm registry trust` subcommands...~%")
    ...
    (multiple-value-bind (base-url stop-server)
      ...
      (unwind-protect
           (progn
             ...
             (let* ((sha (clpm.crypto.sha256:bytes-to-hex
                          (clpm.crypto.sha256:sha256-file distinfo)))
                    (pinned (format nil "sha256:~A" sha)))
               ;; Config includes both a git and a quicklisp registry.
               (write-sexp
                (merge-pathnames "config/config.sxp" clpm-home)
                `(:config :format 1
                  :registries ((:git :url "file:///unused" :name "main" :trust "ed25519:deadbeef")
               ...
               (multiple-value-bind (code stdout stderr)
                 ...
                 (unless (eql code 0)
                   (fail "registry trust list failed: ~D~%stderr:~A" code stderr))
                 (assert-equal
               ...
               ;; refresh errors for git
               (multiple-value-bind (code stdout stderr)
                   (run-cli-captured '("registry" "trust" "refresh" "main"))
                 (declare (ignore stdout))
                 (assert-eql 1 code)
                 (assert-true (search "Quicklisp" stderr :test #'char-equal)
                              "Expected refresh error, got:~%~A" stderr))

               ;; refresh updates pinned quicklisp trust
               (write-text
                distinfo
                (with-output-to-string (s)
                  (format s "system-index-url: ~Adist/systems.txt~%" base-url)
               ...
               (let* ((sha2 (clpm.crypto.sha256:bytes-to-hex
                 ...
                 (multiple-value-bind (code _stdout stderr)
                   ...
                   (declare (ignore _stdout))
                   (unless (eql code 0)
                     (fail "registry trust refresh quicklisp failed: ~D~%stderr:~A" code stderr)))
                 (let* ((cfg (clpm.config:read-config))
                        ...
                        (ref (find "quicklisp" refs
                                   ...
                                   :test #'string=)))
                   (assert-true ref "quicklisp ref missing after refresh")
                   (assert-true (string= pinned2 (clpm.project:registry-ref-trust ref))
                                "Expected pin ~S, got ~S"
...
(format t "  Registry trust command tests PASSED~%")
(format t "~%Registry trust command tests PASSED!~%")
(sb-ext:exit :code 0)
