...
- `2026-05-19` **#024 landed.** New `clpm.log` package owns `*verbose*`, `log-info`, `log-verbose`, `log-error`. The four symbols live in `src/log.lisp`; `clpm.commands` `:use`s the new package and re-exports the same four names so existing callers (e.g. `main.lisp` binding `clpm.commands:*verbose*`) keep working unchanged. `clpm.registry` also `:use`s `clpm.log`, replacing the previous `clpm.commands::log-info` double-colon in `registry/git.lisp` with the inherited bare symbol. Verified `rg "clpm\.commands::" src/ | rg -v "src/commands\.lisp"` returns no hits. Suite: 64/64 green.
- `2026-05-19` **#022 + #023 landed.** README now has a "Quicklisp caveats" subsection explaining: (a) synthetic versions = QL dist date `YYYYMMDD`, (b) `systems.txt`-parsed transitive deps inherit `nil` (any) constraints because QL has no constraint model, (c) per-manifest constraints can override either of the above. Plus a "How resolution works" subsection that documents the actual solver behavior — alphabetical system pick order, highest-version-first candidate ordering with lockfile preference lifted to the front, selective-update mechanics (unlocked-first ordering so propagated constraints land before pinned systems are bound), reason-chain conflict format, and the determinism guarantee. Global Options now lists `--fetch-retries / --fetch-timeout` and `--with-optional / --with-all-optional`. `clpm sbom` row in the commands table now advertises all three formats. `clpm help registry add` carries a brief Quicklisp note pointing at the README section. No code behavior change; help-output test still passes (existing assertions are subset-matches).
- `2026-05-19` **#021 landed.** `extract-archive` now handles `.tar.xz`/`.txz` and `.tar.bz2`/`.tbz2`/`.tbz` in addition to gzip/zip/plain-tar. The compression strategy is now keyword-driven (`:none` / `:gzip` / `:xz` / `:bz2`) feeding both the listing flag (`-tf` / `-tzf` / `-tJf` / `-tjf`) and the extraction flag (`-xf` / `-xzf` / `-xJf` / `-xjf`); the previous `extract-tar-gz` / `extract-tar` distinction collapsed into a single `extract-tar-compressed` plus the existing plain-tar path. Same anti-traversal validation is applied to every variant (the new test exercises this for both xz and bz2). `%ensure-compression-tool` checks for the `xz` / `bzip2` helper binary before invoking tar and raises `clpm-missing-tool-error` with platform-appropriate install hints (`xz-utils` on Debian/Ubuntu, `xz`/`bzip2` on Fedora/Homebrew/choco). `test/extract-formats-test.lisp` does a round-trip extraction of a tar built on-the-fly for each format, a path-traversal rejection check per format, and a sanity check that unsupported formats (e.g. `.7z`) still error. Suite: 64/64 green.
- `2026-05-19` **#020 landed.** `fetch-url` now wraps the underlying downloader in a retry loop with quadratic backoff (`*fetch-backoff-base* * (N-1)^2` seconds between attempts; default base = 1s, so the schedule for the default 3 attempts is 0s/1s/4s). Per-request timeouts default to 60s and propagate to `curl --max-time`, `wget --timeout=...`, and `Invoke-WebRequest -TimeoutSec`. Tunables: `*fetch-retries*`, `*fetch-timeout*` (defvars in `clpm.fetch`), env vars `CLPM_FETCH_RETRIES` / `CLPM_FETCH_TIMEOUT`, and global CLI flags `--fetch-retries N` / `--fetch-timeout SECS`. Two test hooks (`*test-fetcher*`, `*fetch-sleep-fn*`) let `test/fetch-retry-test.lisp` exercise transient-recover, permanent-exhaust, env-override, retries=1 no-retry, and the quadratic backoff schedule deterministically without touching the network or actually sleeping. Suite: 63/63 green.
- `2026-05-19` **#019 landed.** `clpm sbom` now supports three formats: `cyclonedx-json` (unchanged), `cyclonedx-xml` (CycloneDX 1.5 XML — same data, XML serialization with attribute/element escaping), and `spdx-json` (SPDX 2.3 with deterministic SPDXIDs derived from `name-version`, sanitized to the SPDX alphabet). Extracted `%sbom-collect-components` to deduplicate components by `(name, version)` and sort them deterministically; each emitter consumes that list. SPDX uses the lockfile's `generated-at` as `creationInfo.created` so output is byte-stable across runs without injecting wall-clock time. Help text in `print-command-help :sbom` advertises all three formats. `test/sbom-command-test.lisp` extended with assertions for each new format plus a byte-for-byte stability check across consecutive runs and a "format rejected" case for unsupported formats.
- `2026-05-19` Pre-existing test isolation bug in `test/concurrent-state-test.lisp` fixed (not in TRACKER originally; surfaced during full-suite verification). The inter-process upsert assertions compared absolute root counts in `projects.sxp` without sandboxing CLPM state, so accumulated dev paths in `~/.local/share/clpm/projects.sxp` would push the count past 96. Now sets `CLPM_HOME` to a unique `/tmp/clpm-conc-home-<pid>-<rand>/` in the parent (children inherit via `sb-ext:posix-environ`) and reseeds `*random-state*` so consecutive runs pick different dirs.
...
- **The backtracking solver does not revisit previously-decided systems on conflict.** When `propagate-dependencies` adds a new constraint that conflicts with an already-decided system, the conflict bubbles up to the most recent decision frame only. Practical implication for selective update: an unlocked system whose only viable version forces a constraint upgrade on a transitively-required dependency must be DECIDED BEFORE that dependency. We achieve this by ordering unlocked systems first in `next-pending-system` rather than by adding a true conflict-driven backjump. A real PubGrub implementation (see #001 Option B) would not need this hack.
...
### #006 — `[x]` `P1` `cli` `update` Implement selective update in `cmd-update`
...
### #008 — `[x]` `P1` `manifest` `silent-noop` Honor `:optional` dependencies
...
### #011 — `[x]` `P2` `build` `package` Support non-SBCL implementations in `cmd-package`
...
### #017 — `[x]` `P3` `cli` `help` Per-subcommand help beyond `registry`
...
### #019 — `[x]` `P3` `sbom` `format` Add SBOM formats beyond CycloneDX-JSON
...
### #020 — `[x]` `P2` `fetch` `reliability` Retries and timeouts for HTTP fetch
...
### #021 — `[x]` `P3` `fetch` `archive` Support `.tar.xz` and `.tar.bz2`
...
### #022 — `[x]` `P2` `docs` Quicklisp constraint model is implicit
...
### #023 — `[x]` `P3` `docs` Document the actual solver
...
### #024 — `[x]` `P3` `coupling` `clpm.commands::log-info` is a private symbol leaking into `clpm.registry`
